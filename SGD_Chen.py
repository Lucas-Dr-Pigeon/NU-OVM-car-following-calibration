...
class Car_Following():
    ...
    def __calibrate__(self, tr_range = [0.1, 3.0], a_range = [2, 20]):
    ...
    def __SGD_calibrate__(self, learning_rate = 1e-2, momentum = 0.9):
        ...
        optimal_tr = tr 
        min_lse = 1e9
        for tr in np.arange(0.2, 3.0, 0.2): 
            lse = 1e9
            prev_lse = 1e8 
            m = 0
            a = 15.0
...
if __name__ == "__main__":
    ...
    for i, cf in enumerate(cf_samples):
        ...
        try:
            CF = Car_Following(cf)
            CF.__SGD_calibrate_a_tr__()
            a_list_2.append(CF.a)
            tr_list_2.append(CF.tr)
