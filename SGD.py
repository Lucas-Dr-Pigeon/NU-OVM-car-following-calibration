...
class Car_Following():
    ...
    def __calibrate__(self, tr_range = [0.1, 3.0], a_range = [0.5, 20.0]):
        
        ''' initialize min_lse '''
        min_lse = 1e9
        for tr in np.arange(tr_range[0], tr_range[1], 0.1):
            for a in np.arange(a_range[0], a_range[1], 0.1):
    ...
    def __SGD_calibrate__(self, learning_rate = 1e-3, momentum = 0):
        ...
        while abs(prev_lse - lse) > 1e-3:
            ...
            prev_m = m
            m = m * r + (1 - r) * n * grad_a
            if abs(prev_m) < abs(m) and prev_m != 0:
                break
            a = a - m
        ...
        print ("The optimal tr and a are: " + str(tr) + ", " + str(a) + " with LSE " + str(lse))
    ...
    def __rearrange__(self, tr = 1.0, a = 0.08, l = 2):
        ''' get the first frame of the sample '''
        ini_frame = np.min(np.asarray(list(self.cf.keys())))
        ...
        while fol_frame + 100 < end_frame:
            ...
            ''' get variables about the fol_veh '''
            fol_vel = self.cf[fol_frame][self.fol_veh]["vel"]
            fol_loc = self.cf[fol_frame][self.fol_veh]["loc"]
            fol_acc_actual = 10 * (self.cf[fol_frame + 100][self.fol_veh]["vel"] - self.cf[fol_frame][self.fol_veh]["vel"])
            ...
            fol_acc_estimate = a * ( w_opt_vel - fol_vel ) 
            
            ''' the error '''
            err = fol_acc_estimate - fol_acc_actual
    ...
    def __simulate__(self, tr = 1.0, a = 1.85, l = 2):
        ''' get the first frame of the sample '''
        ini_frame = np.min(np.asarray(list(self.cf.keys())))
        end_frame = np.max(np.asarray(list(self.cf.keys())))
